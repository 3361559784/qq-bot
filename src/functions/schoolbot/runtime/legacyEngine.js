...
const { hasDelegatedDecisionRequest, buildDelegatedDecisionRefusal } = require('../policy/gates');
const { getRequestId, parseBodySafely } = require('../http/requestParser');
const { routeNonChatEvents } = require('../http/eventRouter');
const { shouldTreatAsDraw, canRunImageAnalysis } = require('../features/media');
const { simpleVectorize: memorySimpleVectorize, cosineSimilarity: memoryCosineSimilarity } = require('../features/memory');
...
const RUNTIME_CONFIG = getRuntimeConfig(process.env);
...
function simpleVectorize(text, dimension = 50) {
    return memorySimpleVectorize(text, dimension);
}
...
function cosineSimilarity(vecA, vecB) {
    return memoryCosineSimilarity(vecA, vecB);
}
...
// 运行时配置（通过环境变量注入，避免硬编码部署细节）
const GPTSOVITS_API_URL = RUNTIME_CONFIG.gptsovits.apiUrl;
const ARIS_GPT_WEIGHTS = RUNTIME_CONFIG.gptsovits.gptWeights;
const ARIS_SOVITS_WEIGHTS = RUNTIME_CONFIG.gptsovits.sovitsWeights;
const ARIS_REF_AUDIO_PATH = RUNTIME_CONFIG.gptsovits.refAudioPath;
const ARIS_REF_PROMPT_TEXT = RUNTIME_CONFIG.gptsovits.refPromptText;
const ARIS_REF_PROMPT_LANG = RUNTIME_CONFIG.gptsovits.refPromptLang;
...
async function synthesizeWithLocalTTS(text, context) {
    ...
    if (!ARIS_REF_AUDIO_PATH || !ARIS_REF_PROMPT_TEXT) {
        context.log("[Tier 2] 参考音频配置缺失，跳过 TTS");
        return null;
    }
...
const { handlePokeLogic, updateLastBotReply } = createPokeModule({
    config: {
        POKE_GROUP_COUNTING,
...
function getGithubToken() {
    return token || process.env["GITHUB_TOKEN"] || process.env["GH_TOKEN"] || process.env["GITHUB_MODELS_TOKEN"] || "";
}
...
module.exports = {
    ...
    handlePokeLogic,
    updateLastBotReply,
    getLegacyEventConfig: () => ({
        arisDisablePoke: ARIS_DISABLE_POKE,
        botQqId: BOT_QQ_ID
    }),
...
const runLegacyHandler = async (request, context) => {
        // ==========================================
        ...
        try {
            ...
            // 1. 解析消息 (强化版：防注入 + 强力清洗)
            try {
                const parsedBody = await parseBodySafely(request);
            if (parsedBody) {
                body = parsedBody;
...
保持可爱语气,用颜文字点缀 (✨ω✨)`;
        ...
        // Pipeline is still evolving; keep legacy path as default unless explicitly enabled.
        const pipelineEnabled = String(process.env["ARIS_PIPELINE_ENABLED"] || '').toLowerCase() === 'true';
	        if (pipelineEnabled) {
	            const sessionKey = `${dbKey}:${senderId}`;
...
- 明天有 ${sd.tomorrowCourses?.length || 0} 节课`;
        ...
        const drawTriggered = shouldTreatAsDraw(msg, intentResult, INTENT_CONFIDENCE_THRESHOLD);
        ...
        if (canRunImageAnalysis({ imageUrls, mediaReply, isDrawTaskDone })) { // 增加绘图任务完成检测
...
注意：即使在闲聊模式下，也不能编造课程。这是数据准确性的底线。`;
        ...
        async function callAI(messages, systemPrompt, opts = {}) {
            ...
            if (RUNTIME_CONFIG.response.exposeDebugMeta) {
                responsePayload.meta._debug = {
...
module.exports.runLegacyHandler = runLegacyHandler;
