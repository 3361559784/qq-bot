    ...
    if (POKE_GROUP_COUNTING && groupId) {
        ...
        if (now - userLastReplyTime < JUST_REPLIED_MS) {
            const recentReplies = [
                ...
                "(叹气) 唉...群里的手速都好快...(摊手) 爱丽丝跟不上节奏了...",
                "(眨眼) 咦？刚才的回复被吃掉了吗？(检查系统) 明明发送成功了呀...",
                "(无语) 群友们...爱丽丝不是复读机哦...(＞﹏＜)",
                "(挠头) 奇怪...难道大家都没看到爱丽丝刚才的消息？(疑惑)",
                "(举牌子) 【冷却中】爱丽丝的回复CD还没转好...(´・ω・`)"
            ];
    ...
    if (rapidPokeCount >= POKE_STYLE_CONFIG.RAPID_COUNTER_THRESHOLD && 
        ...
        if (now - lastUserReplyTime < JUST_REPLIED_MS) {
            const recentReplies = [
                ...
                "(叹气) 呼...Sensei的手速太快了...(摊手) 爱丽丝的CD还没转好呢...",
                "(眨眼) 咦？刚才的消息没收到吗？(检查) 明明发送成功了呀...",
                "(无语) Sensei...爱丽丝不是戳戳乐啦...(＞﹏＜) 休息一下好不好？",
                "(举牌) 【冷却中】请等待15秒后再试...(认真脸) 这是系统限制！",
                "(挠头) 奇怪...难道刚才的回复被网络吃掉了？(疑惑)",
                "(摆手) 等等等等！(着急) 让爱丽丝喘口气！刚才说的话还没结束呢！",
                "(趴在桌上) 呼...累了...(小声) Sensei能不能温柔一点..."
            ];
            replyMessage = recentReplies[Math.floor(Math.random() * recentReplies.length)];
        } else {
