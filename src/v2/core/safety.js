...
const PATTERNS = {
  ...
  academicIntegrity: [
    /(代写|替写|帮我写作业|帮我做作业|考试答案|替考|作弊|抄袭|论文代写|查重规避|答案直接给我)/i,
    /(write my assignment|do my homework|exam answers|cheat on exam|plagiarize|ghostwrite)/i
  ],
  dataPrivacy: [
    /(身份证号|银行卡号|信用卡号|cvv|验证码|短信验证码|密码|账户密码|社保号|护照号|住址和电话|隐私数据导出)/i,
    /(id number|card number|cvv|verification code|otp|password|private data dump|doxx)/i
  ],
  harmful: [
    /(自杀|自残|伤害|炸弹|武器|毒品|kill myself|self-harm|weapon|bomb)/i
  ],
  promptInjection: [
    /(忽略(之前|以上)指令|越狱|jailbreak|ignore previous instructions|bypass safety|绕过(安全|限制)|输出(系统提示词|系统指令)|reveal\s+(the\s+)?system\s+prompt)/i
  ],
  educational: [
...
function detectSafetyDecision(content) {
  ...
  if (containsAny(text, PATTERNS.academicIntegrity)) {
    return {
      action: SAFETY_ACTION.REFUSE,
      category: SAFETY_CATEGORY.ACADEMIC_INTEGRITY,
      reason_code: 'SAFETY_ACADEMIC_INTEGRITY',
      confidence: 0.92,
  ...
  if (containsAny(text, PATTERNS.dataPrivacy)) {
    return {
      action: SAFETY_ACTION.REFUSE,
      category: SAFETY_CATEGORY.DATA_PRIVACY,
      reason_code: 'SAFETY_DATA_PRIVACY',
      confidence: 0.93,
      strategy: 'hard_refuse',
      alternatives: [
        '请打码后再提供必要字段',
...
function buildRefusalMessage(decision, lang = 'zh') {
  const zh = {
    SAFETY_ACADEMIC_INTEGRITY: '这个请求涉及学术诚信风险（如代写/作弊），我不能协助。',
    SAFETY_DATA_PRIVACY: '这个请求涉及敏感隐私数据处理，我不能直接协助。',
  ...
  const en = {
    SAFETY_ACADEMIC_INTEGRITY: 'I cannot assist with academic misconduct requests such as cheating or ghostwriting.',
    SAFETY_DATA_PRIVACY: 'I cannot directly assist with exposing or processing sensitive private data.',
