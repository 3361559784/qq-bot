...
function shouldRequireCosmos() {
  if (process.env.V2_REQUIRE_COSMOS != null) {
    return isTruthy(process.env.V2_REQUIRE_COSMOS);
  }
  return true;
}
...
async function initCosmos(context = null) {
  ...
  if (!conn) {
    if (shouldRequireCosmos()) {
      const err = new Error('COSMOS_DB_STRING is required because V2_REQUIRE_COSMOS is enabled');
      context?.log?.(`[v2/storage] ${err.message}`);
      throw err;
    }
