Schedule = {
	...
	isShowTag:-1, //是否显示自建标签  为0时显示
	firstRender: true, // 是否为首次渲染 用于设置课表top偏移量时判断
	displayTime: null // 是否需要展示时间
}
var timer;
let opType = 1 // 统一配置
let schoolData = null // 单位数据
...
//				console.log(startpageX +'  '+endpageX +'  '+startpageY+'   '+endpageY)
		...
		if (object && object.typeflag) {
			if (object.typeflag == 'CXWEB_DATA_RELOAD') {
				...
				if (object.info.schoolYear != Schedule.curriculum.schoolYear
					...
					sessionStorage.setItem('last_selected_curriculum', JSON.stringify(localStorageData));
					// 学期或者学年有变化，重新加载数据
    ...
    $('.setyear').click(function() {
		console.log('setyear',Schedule.curriculum.schoolYear,Schedule.curriculum.semester);
        AppUtils.openUrl({
            toolbarType: 1,
            webUrl: window.location.origin + mctx +  '/res/app/curriculum/curriculumHistory.html?v=2&schoolYear='+ Schedule.curriculum.schoolYear +'&semester='+Schedule.curriculum.semester
        });
    });
	// 扫一扫
...
Schedule.updateLessonConfig = function () {
	...
	// 国科大定制 在调用/pc/curriculum/updateLessonConfig接口的时候updateLocationType一定为1
	if(Schedule.curriculum.fid == '35014'){
		param.updateLocationType = 1;
	}
	// 关联班级空间
	$.ajax({
		type: 'POST',
...
Schedule.loadLessons = function(week, queryType, reload) {
	if(localStorage.getItem('last_selected_curriculum')){
		localStorage.removeItem('last_selected_curriculum');
	}
	var localStorageData = sessionStorage.getItem('last_selected_curriculum');
	if (localStorageData) {
		// 	缓存中有记录用户切换后的学期
		try {
	...
	if (url.startsWith('http')) {
		...
		if (Schedule?.curFid?.fidEnc) {
			requestParam.kd_fidenc = Schedule?.curFid?.fidEnc;
		}
		// 如果当前单位不存在，则使用不启用单位展示课表（选择该项后，多个单位的课程将同时展示在课表上）
		if(Schedule?.curFid?.currentCampusId){
			requestParam.currentCampusId = Schedule.curFid.currentCampusId;
		}
		$.getJSON(Schedule.mctx + url, requestParam, function(data){
			...
			if(data.data.curriculum.displayTime){
				Schedule.displayTime = data.data.curriculum.displayTime;
			}

			data = data.data;
			let fidEnc,text,currentCampusId='';//记录接口返回的校区id和单位id
			const params = new URLSearchParams(window.location.search);
			if(data.schools && Array.isArray(data.schools) && data.schools.length){
				// 获取当前单位
				schoolData = data.schools;
				const cur = data.schools.find(item => item.current_fid === 1);
				// 如果当前单位不存在，则使用不启用单位展示课表（选择该项后，多个单位的课程将同时展示在课表上）
				if (cur) {
					requestParam.kd_fidenc = cur.kd_fidenc;
					fidEnc = cur.kd_fidenc;
					text = cur.schoolname;
				}else{
					requestParam.kd_fidenc = data.schools[0].kd_fidenc;
					fidEnc = data.schools[0].kd_fidenc;
					text = data.schools[0].schoolname;
				}
				
	
				if (!Schedule?.curFid?.fidEnc && fidEnc) {
					Schedule.curFid = {
						fidEnc: fidEnc,
						text: text,
					}
					// 在url上更新单位id，校区id和单位名称
					params.set('kd_fidenc', fidEnc || '');
			...
			if(data.curriculum.extend && typeof data.curriculum.extend === 'string'){
				let extent =JSON.parse(data.curriculum.extend)
				if(extent && extent.currentCampusId){
					currentCampusId = JSON.parse(data.curriculum.extend).currentCampusId
				}
				Schedule.curFid.currentCampusId = currentCampusId
				params.set('currentCampusId', currentCampusId || '');
			}
			//只修改 URL，不触发刷新
...
Schedule.renderPage = function(data) {
	...
	// ---------------
	if (data.curriculum.campusLessonTimeConfigArray && data.curriculum.campusLessonTimeConfigArray.length) {
		opType = 2 // 多校区配置
		let currentCampusId,schoolIndex;
		if(data.curriculum.extend && typeof data.curriculum.extend === 'string'){
			currentCampusId = JSON.parse(data.curriculum.extend).currentCampusId
		}
		data.curriculum.campusLessonTimeConfigArray.forEach((item, index) => {
			if(md5(item.name) == currentCampusId){
				schoolIndex = index
			}
		})
		localStorage.setItem('_schedule_school_key', JSON.stringify(schoolIndex || 0))
	}
	if (data.curriculum.weeksLessonTimeConfigArray && data.curriculum.weeksLessonTimeConfigArray.length) {
		opType = 3 // 周次配置
	}
	let copyCurriculum = data.curriculum
	if (opType === 2) {
		let key = localStorage.getItem('_schedule_school_key')
		if (!key) {
			key = 0
			localStorage.setItem('_schedule_school_key', '0')
		} else {
			try {
				key = JSON.parse(key)
				if (key > data.curriculum.campusLessonTimeConfigArray.length) {
					key = 0
				}
			} catch (error) {console.log(error);}
		}
		var campusConfig = data.curriculum.campusLessonTimeConfigArray[key] || {}
		// copyCurriculum = { ...copyCurriculum, ...campusConfig}
		copyCurriculum.earlyMorningSection = campusConfig.earlyMorningSection || 0
		copyCurriculum.morningSection = campusConfig.morningSection || 0
		copyCurriculum.afternoonSection= campusConfig.afternoonSection || 0
		copyCurriculum.eveningSection = campusConfig.eveningSection || 0
		copyCurriculum.lessonTimeConfigArray = campusConfig.timeConfig || []
		
		// 生成切换校区
		genertateSchoolList(data.curriculum.campusLessonTimeConfigArray)
	} else {
		// 隐藏切换校区按钮
		$('.changeSchool').hide()
	}
	if (opType === 3) {
		// 获取当前周次
		let week = copyCurriculum.currentWeek + ''
		for(let item of copyCurriculum.weeksLessonTimeConfigArray){
			let weeks = item.weeks.split(',')
			if(weeks.includes(week)){
				copyCurriculum.earlyMorningSection = item.earlyMorningSection || 0
				copyCurriculum.morningSection = item.morningSection || 0
				copyCurriculum.afternoonSection= item.afternoonSection || 0
				copyCurriculum.eveningSection = item.eveningSection || 0
				copyCurriculum.lessonTimeConfigArray = item.timeConfig || []
			}
		}
	}
	Schedule.curriculum = copyCurriculum;
	// ----------------
	...
	if (Schedule.firstWeekDate != Schedule.curriculum.firstWeekDate && !Schedule.curFid) {
		hasChangeFirstWeekDate = true;
	}
	Schedule.firstWeekDate = Schedule.curriculum.firstWeekDate;
	...
	// 时间区间配置
	if (opType === 2) {
		let index = localStorage.getItem('_schedule_school_key')
		if (index) {
			index = JSON.parse(index)
			if (index > Schedule.curriculum.campusLessonTimeConfigArray.length) {
				index = 0
			}
		} else {
			index = 0
		}
		Schedule.stime = Schedule.curriculum.campusLessonTimeConfigArray[index].timeConfig
	} else if (opType === 1) {
		Schedule.stime = Schedule.curriculum.lessonTimeConfigArray;
		//当lessonTimeConfig的长度和（maxLength+早晨节次）大小不一致时，按照maxLength来渲染课表首页
		if (Schedule.stime.length > Schedule.curriculum.maxLength + Schedule.curriculum.earlyMorningSection){
			Schedule.stime.splice(Schedule.curriculum.maxLength,Schedule.stime.length-(Schedule.curriculum.maxLength+Schedule.curriculum.earlyMorningSection));
		}else {
			//lessonTimeConfig的长度不足节次最大长度时，补充""到数组中
			var len = Schedule.curriculum.maxLength + Schedule.curriculum.earlyMorningSection - Schedule.stime.length;
			for (i = 0; i < len; i++) {
				Schedule.stime.push('');
			}
		}
		if (Schedule.stime.length < Schedule.curriculum.earlyMorningSection + Schedule.curriculum.maxLength) {
		...
		for(var i=0; i<Schedule.curriculum.maxLength; i++) {
			Schedule.stime.push('');
		}
		if (Schedule.stime.length < Schedule.curriculum.earlyMorningSection + Schedule.curriculum.maxLength) {
			// 早晨节次是后面新增的，之前的时间配置里面可能没有，补上早晨的时间
	...
	/*华科定制 非华科显示设置按钮*/
	if (Schedule.isContainsFid(1731) || Schedule.isContainsFid(288823)) {
		$('.rightHead .setting').hide()
	}

	// 西电课表增加备注
	if (
		Schedule.firstRender && 
		(Schedule.isContainsFid(16820) || Schedule.isContainsFid(16820) ||
		Schedule.isContainsFid(146332) || Schedule.isContainsFid(146332))
	) {
		Schedule.firstRender = false
		$('.courseTips').show()
		// 动态增加top偏移量 考虑刘海屏情况
		$('.table').css({
			top: parseInt($('.table').css('top')) + 56
		})
	}
...
function renderCourse(td,rowspan,row,col,lessonId,colorIndex,courseName,teaNameHtml,courseLocHtml,courseOnlineLocHtml,weekHtml,classHtml,time,obj) {
	var showFormState = false
	var extend = JSON.parse(obj.extend)
	var teachLogType = obj?.teachLogType + '' || ''
	var showTeachLog = (teachLogType.length > 0 && teachLogType == 1 && obj.teachLogUseForm && obj.isShowWriteStatus)
	if( obj.isWriteTeachLog2Form != null && obj.isWriteTeachLog2Form!= undefined  && obj.isWriteTeachLog2Form == true ){
		showFormState = true
	}
	if (obj.conflictUuids && obj.conflictUuids.length > 0) {
			...
			$(td)
				...
				.html(
					'<div class="tddiv color' +
						...
						time +
						'</p>'+
						((obj.courseNo == null || obj.courseNo.length == 0) && Schedule.isShowTag == 0?'<p class="courseType">自建</p>':'' )+
						'<p class="courseName">' +
						courseName +
						"</p>" +
						teaNameHtml +
		...
		} else if(Schedule.isContainsFid(1022)){
		$(td)
		.attr("rowspan", rowspan)
		.html(
			'<div class="tddiv color' +
			...
			' >冲突</div>'+
			((obj.courseNo == null || obj.courseNo.length == 0) && Schedule.isShowTag == 0?'<p class="courseType">自建</p>':'' )+
			((!Schedule.isContainsFid(1035)&& showTeachLog && ToolsUtils.isTeacher(obj.role) && obj.courseNo && showFormState == false )?'<div class="formState add" style="">'+ ('未填')+'</div>':'')+
			((!Schedule.isContainsFid(1035)&& showTeachLog && ToolsUtils.isTeacher(obj.role) && obj.courseNo && showFormState == true )?'<div class="formState edit" style="">'+ ('已填')+'</div>':'')+
			'<p class="courseName">' +
			(extend && extend.CourseLevel === '1' ? '<img src="images/clashCourse.svg" class="courseImg" style="background: #1A9CFF;"></img>' : '') +
			(extend && extend.CourseLevel === '2' ? '<img src="images/graduate.svg" class="courseImg"style="background: #1A75FF;"></img>' : '') +
			(getCourseTag(obj) ? '<img class="courseTagImg" src="'+ getCourseTag(obj) + '">' : '') +	
			courseName +
			"</p>" +
			teaNameHtml +
			courseLocHtml +
		...
		// 如果是隐藏冲突，就将对应元素设置为none
		let clashList = $('.clashCourse')
		if(obj.hideConflict && clashList.length > 0){
			clashList.each(function(){
...
function addCourse(row,col,rowspan,courseName,courseLoc,colorIndex,lessonId,
	...
	if (onlineLocation) {
		if (ToolsUtils.checkUrl(onlineLocation)) {
			...
			// 1731 腾讯会议更改为线上会议
			if (Schedule.isContainsFid(1731)) {
				onlineLocation = '线上会议'
			}
	...
	if ($(td).length > 0) {
		...
		let tempLength = row+(rowspan-1) - Schedule.stime.length
		var height= ''; //彩色框高度
		if(tempLength > 0 ){
			height= ((rowspan - tempLength)*1.4-0.1-0.16) +'rem'
		}else{
			height= (rowspan*1.4-0.1-0.16) +'rem'
		}
...
function initTbale(){
	...
	for(var i=0;i<Schedule.stime.length;i++){
		...
		//20200821 加每节课起止时间
		var stimeHtml = '';
		if (Schedule.stime[i] && Schedule.displayTime == 1) {
...
function getWeekDate(startDate,weekNum){
	...
	for(var i=0;i<weekNum;i++){
		...
		for(var j=0;j<7;j++){
			...
			week[weekindex[i]].push(date.getFullYear() +'-'+ ('0'+(date.getMonth()+1)).slice(-2) +'-'+ ('0'+date.getDate()).slice(-2));
			}
...
Schedule.openDetailPage = function(lesson) {
	...
	var endIndex = lesson.beginNumber+lesson.length+Schedule.curriculum.earlyMorningSection-2;
	if (endIndex >= Schedule.curriculum.lessonTimeConfigArray.length) {
		endIndex = Schedule.curriculum.lessonTimeConfigArray.length-1;
	}
	var lessonEndTime = Schedule.curriculum.lessonTimeConfigArray[endIndex].split('-')[1];
	...
	} else if(lesson.meetCode && !ToolsUtils.isTeacher(lesson.role) && !Schedule.isBetweenLimitTime(lesson)) {
		// 上课前多少分钟 学生能进入课堂限制
		AppUtils.clientMessageDisplay('上课前后' + lesson.joinKtLimit + '分钟才允许进入课堂');
		return
	}
	var attendanceUrl = '';
	...
	if (lesson && lesson.courseId && lesson.classId
		...
		// 西安工业大学定制详情页显示课程录播资源
		// if (lesson.fid == 18328 || Schedule.isContainsFid(18328)){
		// 	data.classRoomLive = {}
		// 	let url = 'https://newes.chaoxing.com/live/viewCourseLiveFromTimeTable'
		// 	let startTime = `${date} ${lessonBeginTime}:00`
		// 	let params = `courseNo=${encodeURIComponent(lesson.courseNo)}&classNo=${encodeURIComponent(lesson.classNo)}&startTime=${encodeURIComponent(startTime)}`
		// 	let webUrl = `${url}?${params}`
		// 	data.classRoomLive.liveButton = {
		// 		"title": "课程录播资源",
		// 		"webUrl": webUrl,
		// 		"description": "详情"
		...
		if (lesson.teachLogUseForm && ToolsUtils.isTeacher(lesson.role) && lesson.courseNo && isClientSupportTeaLog()) {
...
// 判断是否能够进入课堂
Schedule.isBetweenLimitTime = function(lesson){
	var beginTime = Schedule.stime[lesson.beginNumber-1].split('-')[0],
		endTime = Schedule.stime[lesson.beginNumber+lesson.length-2].split('-')[1],
		wholeDay = lesson.wholeDay,
		joinKtLimit = lesson.joinKtLimit

	//未设置进入课堂时间限制 不做判断
	if (!beginTime || !endTime || joinKtLimit === null || joinKtLimit === undefined){
		return true;
	}

	var startTimeStr = (wholeDay + ' '+ beginTime).replace(/-/g, '/'),
		endTimeStr = (wholeDay + ' '+ endTime).replace(/-/g, '/'),
		beginDate = new Date(startTimeStr).getTime() - joinKtLimit * 60 * 1000,
		endDate = new Date(endTimeStr).getTime() + joinKtLimit * 60 * 1000,
		currentTime = Date.now()

	return currentTime >= beginDate && currentTime <= endDate
}
...
Schedule.getFormEditUrl = function(lesson, protocolData) {
	...
	var url = Schedule.domain + Schedule.mctx + '/curriculum/getFormAddOrEditUrl?wholeDay='+lesson.wholeDay
		...
		$.ajax({
			...
			success: function(data){
				if(data && data.result == 1 && data.data) {
					if (data.data.formEditUrl) {
						//表单编辑页地址
						protocolData.unitNoteUrl = data.data.formEditUrl;
						protocolData.courseNoteTitle = '详情';
					}else {
						//表单新增页地址
						lesson.formAddUrl = data.data.formAddUrl;
...
$(function(){
	...
	if (!/Android|harmony_phone|webOS|iPhone|iPod|BlackBerry/i.test(navigator.userAgent)
		&& !Schedule.isKeTang) {
	...
	Schedule.event();
	Schedule.isShowTag = ToolsUtils.getUrlParameter('isShowTag') ? parseInt(ToolsUtils.getUrlParameter('isShowTag')) :-1
	if (isSupportImport()){
		//显示导入按钮
		$('.settingMask .import').show();
	}
	const params = new URLSearchParams(window.location.search);
	let fidEnc = params.get('kd_fidenc') || '';
	let currentCampusId = params.get('currentCampusId') || '';
	Schedule.curFid = {
		fidEnc: fidEnc,
...
//解决history.back()不刷新页面的问题
	...
	if(fromsetting){
		...
		var semster = window.sessionStorage.getItem('semster');
	}else{
...
function getClashItemHtml(colorName, item, wrapClass) {
	...
	let tempStr = '<div class="item '+ wrapClass + ' '+ colorName +'"><div class="titleBox">'+
			...
			(ToolsUtils.isTeacher(item.role) == true ?'<div class="class">班级:  '+ item.className +'</div>':'<div class="teacher">教师  '+ (item.teacherName ? item.teacherName : '') +'</div>')+
...
function genertateSchoolList(curriculumArray) {
	const arr = curriculumArray.map(item => item.name || '')
	let index
	const i = localStorage.getItem('_schedule_school_key')
	if (i) {
		index = JSON.parse(i)
	}
	...
	houseTypeObj = new MobileSelect({
		...
		position: [index],
		ensureBtnColor:' rgba(0, 153, 255, 1)',
		cancelBtnColor: 'rgba(0, 153, 255, 1)',
		callback: function (index, data) {

			//? mobile-selec的github地址：https://github.com/onlyhom/mobile-select/blob/master/docs/README-CN.md
			localStorage.setItem('_schedule_school_key', String(index))
			const params = new URLSearchParams(window.location.search);
			params.set('currentCampusId', md5(data[0]));
...
// 	const params = new URLSearchParams(window.location.search);
// 	let curFidEnc = Schedule?.curFid?.fidEnc || '';
// 	$.get(mctx + '/school/getMySchools', function(data){
// 		if (!data || data.result != 1 || !data.data || !data.data.schools) {
// 			return;
// 		}
// 		data = data.data;
// 		var schools = data?.schools || []
// 		schools.unshift({schoolname:'不启用分单位展示课表（选择该项后，多个单位的课程将同时展示在课表上）',kd_fidenc:''})
// 		let index = 0
// 		index = schools.findIndex(item => item.kd_fidenc == curFidEnc)
// 		if (index && index >= schools.length) {
// 			index = 0
// 		}
// 		unitTypeObj = new MobileSelect({
// 			trigger: '#unit_type',
// 			title: '选择单位',
// 			wheels: [
// 					{ data: schools }
...
// 				let i = index[0]
// 				let item = schools[i]
// 				params.set('kd_fidenc', item.kd_fidenc);
// 				location.search = params.toString();
// 			}
// 		});
// 		unitTypeObj.show()
...
function getMySchools() {
	const params = new URLSearchParams(window.location.search);
	let curFidEnc = Schedule?.curFid?.fidEnc || '';
	if(schoolData && Array.isArray(schoolData) && schoolData.length){
		let index = 0
		index = schoolData.findIndex(item => item.kd_fidenc == curFidEnc)
		if (index && index >= schoolData.length) {
			index = 0
		}
		unitTypeObj = new MobileSelect({
			trigger: '#unit_type',
...
// 获取课程标签
function getCourseTag(obj) {
	// 1807 单位定制课表卡片显示课程标签
	if(!Schedule.isContainsFid(1807)) return
	var iconSrc = ''
	try {
		var extendInfo = JSON.parse(obj.extend)
		switch(extendInfo.kcbq) {
			case '市级优质课':
				iconSrc = './images/s-cityExcellent.png'
				break
			case '校级优质课':
				iconSrc = './images/s-schoolExcellent.png'
				break
			case '校级规范课':
				iconSrc = './images/s-schoolStandard.png'
				break
			case '国家级一流课程':
				iconSrc = './images/s-counteryTop.png'
				break
		}
	} catch(e) {
		console.log('kcbq解析失败', e)
	}
	console.log(extendInfo.kcbq, iconSrc)
	return iconSrc
}